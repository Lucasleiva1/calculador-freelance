...
const workspace: Workspace = { project, quote: { id: "preview-quote", projectId: project.id, version: 1, status: "draft", currency: "USD", notes: null, selectedPriceKind: "recommended", selectedPriceMinor: null, floorTotalMinor: null, recommendedTotalMinor: null, premiumTotalMinor: null, snapshotRevision: 0, savedAt: null, archivedAt: null, createdAt: timestamp, updatedAt: timestamp }, services: [service] };
...
export function ResponsivePreview() {
  ...
  const projectResult: ProjectResult = { services: [{ service: currentService, result }], totalMinor: result.effectiveSubtotalMinor, totalHours: 0, externalCostsMinor: result.externalCostsMinor, effectiveHourlyMinor: null, marginMicros: null, pricingTiers: { floorMinor: result.calculatedSubtotalMinor, recommendedMinor: result.suggestedSubtotalMinor, premiumMinor: result.pricingTiers?.premium.totalMinor ?? result.suggestedSubtotalMinor }, unpricedCount: result.status === "ready" ? 0 : 1, isPartial: result.status !== "ready" };
  ...
  return <div className="app-shell">
    ...
    <div className="app-body">
      ...
      <WorkspaceView workspace={currentWorkspace} settings={settings} pricing={pricing} result={projectResult} presets={[]} statuses={{ [service.id]: "saved" }} errors={{}} activeServiceId={service.id} onActiveService={() => undefined} onAddService={async () => undefined} onVideoChange={() => undefined} onProgrammingChange={() => undefined} onGenericEngineChange={(_, next) => setProductConfig(next as ProductConfiguration)} onFinalPriceChange={() => undefined} onTitleChange={() => undefined} onDeleteService={async () => undefined} onMoveService={async () => undefined} onRetry={() => undefined} onSavePreset={async () => undefined} onUpdatePreset={async () => undefined} onDeletePreset={async () => undefined} onRestorePreset={async () => undefined} market={null} marketJob={null} onUpdateMarket={async () => undefined} onCancelMarket={async () => undefined} onSaveQuote={async () => undefined} />
    </div>
  </div>;
}
